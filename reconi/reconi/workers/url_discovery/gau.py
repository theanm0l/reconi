"""GAU-style URL aggregator — combines Wayback, CommonCrawl, and AlienVault OTX sources."""
import asyncio
import random
...
class GauModule(ReconModule):
    ...
    description = "Aggregate URLs from multiple sources: Wayback Machine, CommonCrawl, AlienVault OTX"
    ...
    async def _fetch_wayback(self, client: httpx.AsyncClient, domain: str, seen: set[str]) -> list[Finding]:
        ...
        try:
            url = f"https://web.archive.org/cdx/search/cdx?url=*.{domain}/*&output=text&fl=original&collapse=urlkey&limit=10000"
            ...
            for line in resp.text.strip().splitlines():
                ...
                if line and not line.startswith("original") and line not in seen:
                    ...
                    results.append(Finding(
                        ...
                        type="url",
                        value=line,
                        context="Found via Wayback Machine (GAU aggregator)",
                        url_found_on=url,
                        severity="info",
                        confidence=0.7,
                    ))
            ...
            for entry in data.get("url_list", []):
                ...
                if entry_url and entry_url not in seen:
                    ...
                    results.append(Finding(
                        ...
                        type="url",
                        value=entry_url,
                        context="Found via AlienVault OTX",
                        url_found_on=url,
                        severity="info",
                        confidence=0.7,
                        raw=entry,
...
@celery_app.task(name="url_discovery.gau")
def run_gau_task(job_id: str, target: str):
    async def _run():
        db = SessionLocal()
        try:
